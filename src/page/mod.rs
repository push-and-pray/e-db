...
impl Page {
    pub fn new(position: usize, page_size: usize) -> Self {
        Self {
            position,
    ...
    pub fn from_vec(position: usize, data: Vec<u8>, page_size: usize) -> Self {
        assert_eq!(data.len(), page_size, "Page size mismatch");
        Self { position, data }
    }
...
impl PageManager {
    pub fn read_page(&mut self, position: usize) -> Result<Page, io::Error> {
        ...
        let offset = (position * self.page_size).try_into().unwrap();

        self.file.seek(SeekFrom::Start(offset))?;
        self.file.read_exact(&mut buf)?;

        Ok(Page::from_vec(position, buf, self.page_size))
    }
    ...
    pub fn write_page(&mut self, page: &Page) -> Result<(), io::Error> {
        ...
        let offset = (page.position * self.page_size).try_into().unwrap();
        self.file.seek(SeekFrom::Start(offset))?;
...
mod test {
    ...
    #[test]
    fn page_init() {
        let page = Page::new(0, PAGESIZE);
        assert!(page.data.iter().all(|&byte| byte == 0));
    }

    #[test]
    fn page_from_vec() {
        let page = Page::from_vec(0, vec![1; PAGESIZE], PAGESIZE);
    ...
    fn page_from_wrong_vec() {
        let _page = Page::from_vec(0, vec![1; PAGESIZE * 2], PAGESIZE);
    }

    #[test]
    fn page_read() {
        let page = Page::from_vec(0, vec![2; PAGESIZE], PAGESIZE);
        assert!(page.read().iter().all(|&byte| byte == 2));
    }

    #[test]
    fn page_mutate() {
        let mut mutable_page = Page::new(2, PAGESIZE);
    ...
    fn page_manager_read_write() {
        ...
        let page = Page::from_vec(0, vec![3; PAGESIZE], PAGESIZE);
    ...
    fn page_manager_append() {
        ...
        for i in 0..=3 {
            let page = Page::from_vec(0, vec![i as u8; PAGESIZE], PAGESIZE);
    ...
    fn page_manager_read_write_position() {
        ...
        for i in 0..=10 {
            let page = Page::from_vec(i, vec![i as u8; PAGESIZE], PAGESIZE);
            manager.write_page(&page).unwrap();
        }
